...
ALTER TABLE "SpyLocation" ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Players can view their games"
  ...
  USING (
    EXISTS (
      ...
      AND "Player"."userId" = get_current_user_id()
    )
  );
...
CREATE POLICY "Lobby creators can view lobby games"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM "Lobby"
      WHERE "Lobby"."id" = "Game"."lobbyId"
      AND "Lobby"."creatorId" = get_current_user_id()
    )
  );
...
CREATE POLICY "Users can view players in their games"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM "Player" AS "MyPlayer"
      WHERE "MyPlayer"."gameId" = "Player"."gameId"
...
CREATE INDEX IF NOT EXISTS "idx_player_userid_gameid" ON "Player"("userId", "gameId");
CREATE INDEX IF NOT EXISTS "idx_game_lobbyid" ON "Game"("lobbyId");
CREATE INDEX IF NOT EXISTS "idx_lobby_creatorid" ON "Lobby"("creatorId");
