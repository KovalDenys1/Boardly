...
const C4_DISC_RED = 'var(--bd-coral)'
const C4_DISC_YELLOW = 'var(--bd-sun)'
const C4_DISC_EMPTY = 'var(--bd-bg2)'
const C4_ROWS = 6
const C4_COLS = 7

function ReadOnlyConnectFourView({ state, players }: { state: Record<string, any>; players: GamePlayer[] }) {
  const data = (isRecord(state.data) ? state.data : {}) as Record<string, any>
  const board: (1 | 2 | null)[][] = Array.isArray(data.board) ? data.board : Array.from({ length: C4_ROWS }, () => Array(C4_COLS).fill(null))
  ...
  const discColor = (disc: 1 | 2 | null) => (disc === 1 ? C4_DISC_RED : disc === 2 ? C4_DISC_YELLOW : C4_DISC_EMPTY)
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        style={{
          background: isFinished ? 'var(--bd-ink)' : discColor(isFinished ? null : currentDisc),
      ...
      <div
        style={{
          ...
          gridTemplateRows: `repeat(${C4_ROWS}, 1fr)`,
          gap: 6,
          maxWidth: 360,
          margin: '0 auto',
        }}
...
function ReadOnlySpectatorBoard({
  ...
  switch (gameType) {
    ...
    case 'connect_four':
      return <ReadOnlyConnectFourView state={parsedState} players={players} />
    default:
