...
export default function AliasLobbiesPage() {
  ...
  const loadLobbies = useCallback(async () => {
    try {
      ...
      if (data.error) {
        clientLogger.warn('Alias lobbies loaded with error:', data.error)
      }
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated' || isGuest) {
      ...
      const initSocket = async () => {
        ...
        clientLogger.log('🔌 Connecting to Socket.IO for Alias lobby list:', url)
        ...
        if (!socketAuth) {
          clientLogger.warn('Skipping Alias lobby socket connection: auth payload unavailable')
          return
        }
        ...
        socket.on('connect', () => {
          clientLogger.log('✅ Socket connected for Alias lobby list')
          socket?.emit('join-lobby-list')
        })

        socket.on('lobby-list-update', () => {
          clientLogger.log('📡 Alias lobby list update received')
          loadLobbies()
        })

        socket.on('disconnect', () => {
          clientLogger.log('❌ Socket disconnected from Alias lobby list')
        })
      }
      void initSocket()
      ...
      return () => {
        ...
        if (socket && socket.connected) {
          clientLogger.log('🔌 Disconnecting socket from Alias lobby list')
    ...
    <div className="page-shell bg-gradient-to-br from-orange-400 via-red-500 to-pink-600">
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
        ...
        <div className="mb-4 sm:mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-0">
          <div className="flex-1">
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-2 drop-shadow-lg">
              🗣️ {t('games.alias.lobbies.title')}
            </h1>
            <p className="text-sm sm:text-base lg:text-xl text-white/90">
              {isAuthenticated ? t('games.alias.lobbies.subtitle') : t('games.alias.lobbies.subtitleGuest')}
            </p>
          </div>
          <button
            onClick={() => router.push('/games')}
          ...
          >
            ← {t('games.alias.lobbies.backToGames')}
          </button>
        </div>

        {!isAuthenticated && (
          <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-white/10 border border-white/20 rounded-xl text-white/90">
            <p className="font-semibold text-sm sm:text-base">{t('games.alias.lobbies.wantToPlay')}</p>
            <p className="text-xs sm:text-sm mt-1">
              {t('games.alias.lobbies.wantToPlayDesc')}
            </p>
            <div className="mt-3 flex flex-col xs:flex-row gap-2 sm:gap-3">
              <button
                onClick={() => router.push('/auth/login?returnUrl=/games/alias/lobbies')}
                className="px-4 py-2 bg-white text-orange-600 rounded-lg font-bold hover:bg-orange-50 transition-colors text-sm sm:text-base"
              >
                {t('games.alias.lobbies.signIn')}
              </button>
              <button
                onClick={() => router.push('/auth/register?returnUrl=/games/alias/lobbies')}
              ...
              >
                {t('games.alias.lobbies.createAccount')}
              </button>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
          {/* Create Lobby Card */}
          <div
            className="bg-gradient-to-br from-orange-500 to-red-600 rounded-2xl shadow-2xl p-5 sm:p-8 text-white hover:shadow-3xl transition-all hover:scale-105 cursor-pointer border-2 sm:border-4 border-white/20"
            onClick={() => {
              if (!isAuthenticated) {
                router.push(`/auth/login?returnUrl=${encodeURIComponent('/lobby/create?gameType=alias')}`)
                return
              }
              router.push('/lobby/create?gameType=alias')
            }}
          >
            <div className="flex items-center justify-between mb-3 sm:mb-4">
              ...
              <div className="px-2 sm:px-3 py-1 bg-white/20 backdrop-blur-sm rounded-full text-xs font-bold">
                {t('games.alias.lobbies.newGame')}
              </div>
            </div>
            <h2 className="text-2xl sm:text-3xl font-bold mb-2 sm:mb-3">{t('games.alias.lobbies.createNewLobby')}</h2>
            <p className="text-white/90 mb-4 sm:mb-6 text-sm sm:text-base lg:text-lg">{t('games.alias.lobbies.createDescription')}</p>
            <div className="flex items-center text-white font-bold text-base sm:text-lg">
              <span>{t('games.alias.lobbies.createNow')}</span>
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-2xl shadow-lg p-5 sm:p-8 hover:shadow-xl transition-shadow border-2 border-white/20">
            <h2 className="text-xl sm:text-2xl font-bold text-white mb-3 sm:mb-4">🔍 {t('games.alias.lobbies.quickJoin')}</h2>
            <p className="text-xs sm:text-sm text-white/80 mb-4 sm:mb-6">
              {t('games.alias.lobbies.quickJoinDesc')}
            </p>
            <div className="flex flex-col xs:flex-row gap-2 sm:gap-3">
              <input
                type="text"
                placeholder={t('games.alias.lobbies.enterCode')}
              ...
              <button
                ...
                className="px-6 sm:px-8 py-2 sm:py-3 bg-white text-orange-600 rounded-xl font-bold hover:bg-orange-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all hover:scale-105 shadow-lg text-sm sm:text-base"
              >
                {t('lobby.join')}
              </button>
            </div>
            {!isAuthenticated && (
              <p className="text-xs text-white/70 mt-3">
                {t('games.alias.lobbies.signInToJoin')}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 shadow-xl border border-white/20">
          <h2 className="text-white text-2xl font-bold mb-6 flex items-center justify-between">
            <span>🎮 {t('games.alias.lobbies.activeLobbies')}</span>
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-white/70 text-lg mb-6">
                {t('games.alias.lobbies.noLobbiesTitle')}
              </p>
              {isAuthenticated && (
                <button
                  onClick={() => router.push('/lobby/create?gameType=alias')}
                  className="px-6 py-3 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-xl font-bold hover:shadow-lg transition-all hover:scale-105"
                >
                  {t('games.alias.lobbies.createFirstLobby')}
                </button>
              )}
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <span className="text-xs bg-orange-500 text-white px-3 py-1 rounded-full font-mono font-bold shadow-lg">
                        {lobby.code}
                      </span>
                    </div>
                    ...
                    <div className="text-white/80 text-sm mb-4 flex items-center">
                      ...
                      <span className="truncate">
                        {t('games.alias.lobbies.host')}: {lobby.creator.username || lobby.creator.email?.split('@')[0] || 'Anonymous'}
                      </span>
                    </div>
