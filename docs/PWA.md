...
- There are two opt-in surfaces: the checkbox at `/profile → Settings → Notifications`, and
  ...
  device already has a subscription, within 30 days of a dismissal, or within a day of an
  attempt that failed. A failed `/api/push-subscriptions` POST drops the browser
  subscription again (`lib/push-subscription.ts`) – an orphan left behind would read as
  "already subscribed" and silence the ask on that device for good.
- All three VAPID variables must be set together. The public key is inlined at build time,
  ...
  its own, so both surfaces write the preference as well as the subscription, and both
  check that write landed before telling the player notifications are on.
- **`unsubscribedAll` is the email switch and does not gate push.** `/profile` shows it as
  "Email Notifications", `/api/notifications/unsubscribe?type=all` says it stops notification
  emails, and every other reader of the column (`lib/turn-reminders.ts`,
  `lib/notification-queue.ts`, the lobby invite route) uses it that way. `sendPushNotification`
  read it too until #984's review, which meant a player who had turned email off could accept
  either push opt-in, be told it was on, and never receive anything. Push is turned off by the
  `/profile` push checkbox, which unsubscribes the browser and writes `pushNotifications: false`.
